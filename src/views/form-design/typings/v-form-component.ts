 ...
 * 表单配置
 */
export type PickAntFormConfig = Pick<FormProps, 'size' | 'disabled' | 'labelWidth' | any>;
...
export interface AForm {
   ...
   * @type Function
   */
  validate: <T = any>(names?: namesType, option?: any) => Promise<T>;

  /**
   * validate one or several form items
   * @type Function
   */
  validateField: (name: string, value: any, rules: Record<string, unknown>[], option?: any) => Promise<any[]>;
  /**
